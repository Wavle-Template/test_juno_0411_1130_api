 ...
 */
@Resolver()
export class UserSocialKakaoLoginResolver {
  constructor(
    public configService: ConfigService,
    public kakaoLoginService: UserSocialKakaoLoginService,
  ...
  async signInKakao(
    @Args("accessToken", { description: "카카오 접근 토큰" }) accessToken: string,
  ): Promise<AuthTokenResponse> {
    const socialId = await this.kakaoLoginService.validate(accessToken).catch(() => {
      throw new AuthenticationError("잘못된 카카오 계정입니다.");
    });
    const user = (await this.userSocialService.findOne(UserSocialType.KAKAO, socialId));
  ...
  async signUpKakao(
    ...
    const user = await this.userSocialService.createUser(UserSocialType.KAKAO, socialId, tempData);
