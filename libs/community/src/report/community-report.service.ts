...
@Injectable()
export class CommunityReportService extends CRUDService<CommunityReportEntity> {
    constructor(@InjectRepository(CommunityReportEntity) communityReportRepository: Repository<CommunityReportEntity>) {
        super(communityReportRepository);
    }

    async updateFiles(id: string, fileIds: string[]): Promise<CommunityReportEntity> {
        const post = (await this.repository.findOne({
            where: { id: id },
            relations: ["files"]
        })) as DeepPartial<CommunityReportEntity>;
    ...
    async getPageInfoByUserId(
        userId: string,
        edges: Edge<CommunityReportEntity>[],
        args: PaginationArgs,
    ) {
        let builder = this.getQueryBuilder("reports")
            .leftJoin("reports.author", "author")
